...
def load_data(load_in:str, load_raw_out:str, metadata_in:str, load_meta_out:str):
    ...
    data = sc.read_10x_mtx(load_in, var_names = 'gene_symbols', make_unique=True, cache=False)
    ...
    raw_metadata = old_raw_metadata.drop(labels={'n_genes', 'scrublet_score', 'bh_doublet_pval', 'cell_caller', 'cluster_scrublet_score', 'doublet_pval', 'mt_prop', 'n_counts', 'n_genes', 'annot', 'hierarchy1'}, axis='columns')
    #remove? checking structure 
    ...
    adata.obs = adata.obs.merge(merged_df, left_on='index1', right_on='index1', how='left')
    ...
    samples_to_fill_indices = adata.obs.index[adata.obs['sanger_id'].isin(['HCA_rFSKI14449639', 'HCA_rFSKI14449640','HCA_rFSKI14449641', 'HCA_rFSKI14449642','HCA_rFSKI14449635', 'HCA_rFSKI14449636','HCA_rFSKI14449637', 'HCA_rFSKI14449638' ])]
    ...
    samples_to_fill_indices = adata.obs.index[adata.obs['sanger_id'].isin(['HCA_rFSKI14449635' ])]
    adata.obs.loc[samples_to_fill_indices, 'sorting'] = "CD45P"
    adata.obs.loc[samples_to_fill_indices, 'chemistry_sorting'] = "SC5P-R2_CD45P"

    samples_to_fill_indices = adata.obs.index[adata.obs['sanger_id'].isin(['HCA_rFSKI14449636' ])]
    ...
    samples_to_fill_indices = adata.obs.index[adata.obs['sanger_id'].isin(['HCA_rFSKI14449637' ])]
    adata.obs.loc[samples_to_fill_indices, 'sorting'] = "CD45NCD34N"
    adata.obs.loc[samples_to_fill_indices, 'chemistry_sorting'] = "SC5P-R2_CD45NCD34N"

    samples_to_fill_indices = adata.obs.index[adata.obs['sanger_id'].isin(['HCA_rFSKI14449638' ])]
    ...
    samples_to_fill_indices = adata.obs.index[adata.obs['sanger_id'].isin(['HCA_rFSKI14449639' ])]
    adata.obs.loc[samples_to_fill_indices, 'sorting'] = "CD45P"
    adata.obs.loc[samples_to_fill_indices, 'chemistry_sorting'] = "SC5P-R2_CD45P"

    samples_to_fill_indices = adata.obs.index[adata.obs['sanger_id'].isin(['HCA_rFSKI14449640' ])]
    adata.obs.loc[samples_to_fill_indices, 'sorting'] = "CD45NCD34N"
    adata.obs.loc[samples_to_fill_indices, 'chemistry_sorting'] = "SC5P-R2_CD45NCD34N"

    samples_to_fill_indices = adata.obs.index[adata.obs['sanger_id'].isin(['HCA_rFSKI14449641' ])]
    adata.obs.loc[samples_to_fill_indices, 'sorting'] = "CD45NCD34N"
    adata.obs.loc[samples_to_fill_indices, 'chemistry_sorting'] = "SC5P-R2_CD45NCD34N"

    samples_to_fill_indices = adata.obs.index[adata.obs['sanger_id'].isin(['HCA_rFSKI14449642' ])]
    adata.obs.loc[samples_to_fill_indices, 'sorting'] = "CD45NCD34P"
    adata.obs.loc[samples_to_fill_indices, 'chemistry_sorting'] = "SC5P-R2_CD45NCD34P"

    #### Next donor round
    samples_to_fill_indices = adata.obs.index[adata.obs['sanger_id'].isin([
      'WS_wEMB13942138',
    'WS_wEMB13942139',
    'WS_wEMB13942140',
    'WS_wEMB13942141',
    ...
    columns_to_fill = [ 'pcw', 'chemistry', 'sorting', 'chemistry_sorting',  'donor', 'gender']  # Add more columns if needed
    ...
    for sanger_id, group in adata.obs.groupby('sanger_id'):
        for column in columns_to_fill:
            ...
            adata.obs.loc[group.index, column] = adata.obs.loc[group.index, column].fillna(mode_value)
    ...
    for sanger_id, group in adata.obs.groupby('sanger_id'):
        for column in columns_to_fill:
            ...
            if not mode_values.empty:
                ...
                print(f"Mode is empty for '{column}' in group '{sanger_id}'. Handle this case as needed.")
